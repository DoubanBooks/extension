...
* **Firefox 上架校验修复（data_collection_permissions）**：`manifest.json` 的 `browser_specific_settings.gecko` 新增 `data_collection_permissions`（`data_collection: true`，`required: ["technical"]` 表示功能依赖的技术数据如查询所用 ISBN，仅向第三方书商 API 传输查价，并声明 `extensions_tos_url` / `extensions_privacy_policy_url`）。原因：AMO 对所有新 Firefox 扩展强制要求该字段，且 `required` 至少需含 1 项。注意：两个 URL 当前为占位（`douban-books-plus.pages.dev/privacy.html`），需替换为真实可访问的隐私政策页面以满足人工审核。
* **Firefox 上架校验修复**：
